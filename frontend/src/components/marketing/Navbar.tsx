    ...
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 w-full bg-white/80 backdrop-blur-sm border-b border-[#E5E5E5] transition-all duration-300">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 md:px-8 py-4">
          ...
          <Link
            href="/"
            className="font-display text-xl font-black tracking-tight text-[#0A0A0A] uppercase hover:opacity-70 transition-opacity"
          >
            {t.rich("logo", {
              os: (chunks) => <span className="text-[#0A0A0A]">{chunks}</span>,
            })}
          </Link>
            ...
            <div className="hidden sm:flex items-center gap-3">
              <Show when="signed-in">
                <Link href="/dashboard" prefetch={false}>
                  <Button
                    size="sm"
                    className="rounded-full bg-[#0A0A0A] text-white hover:bg-[#333] text-xs font-semibold uppercase tracking-wide px-4"
                  >
                    {t("dashboard")}
                  </Button>
                </Link>
