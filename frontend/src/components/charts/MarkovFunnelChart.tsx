...
/**
 * Generates a realistic Markov funnel from channel names.
 * Each channel name becomes a node; probabilities are derived from
 * the channel's relative spend share.
 */
export function generateMockMarkovData(
  channelNames: string[]
): MarkovFunnelData {
  // Always include the terminal states
  const allNodes: MarkovNode[] = [
    { id: "organic", label: "Organic", trafficShare: 0.25 },
    ...channelNames.map((name, i) => ({
      id: name.toLowerCase().replace(/\s+/g, "_"),
      label: name,
      trafficShare: 0.5 / channelNames.length + (i === 0 ? 0.1 : 0),
    })),
    { id: "retargeting", label: "Retargeting", trafficShare: 0.15 },
    { id: "converted", label: "Converted", trafficShare: 0.12 },
    { id: "null", label: "Churned", trafficShare: 0.08 },
  ];

  // Build a plausible transition table
  ...
  channelNames.forEach((name, i) => {
    edges.push({
      from: "organic",
      to: name.toLowerCase().replace(/\s+/g, "_"),
      probability: 0.6 / channelNames.length + (i === 0 ? 0.05 : 0),
    });
  });
  edges.push({ from: "organic", to: "null", probability: 0.4 });

  // Paid channels → retargeting / conversion / churn
  channelNames.forEach((name) => {
    const id = name.toLowerCase().replace(/\s+/g, "_");
    edges.push({ from: id, to: "retargeting", probability: 0.35 });
    edges.push({ from: id, to: "converted", probability: 0.25 });
    edges.push({ from: id, to: "null", probability: 0.4 });
  });

  // Retargeting → conversion / churn
  edges.push({ from: "retargeting", to: "converted", probability: 0.55 });
  edges.push({ from: "retargeting", to: "null", probability: 0.45 });
